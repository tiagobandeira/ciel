...
- analisar as ações definidas: se houver múltiplas chamadas consecutivas à mesma tool, perguntar ao usuário se quer ativar o agrupamento — sugerir o valor adequado para o campo grupo (nome da tool ou "tudo")  [tool: entrevista_interativa]
- conduzir entrevista sobre o resultado esperado: o que deve ser entregue ao final da task  [tool: entrevista_interativa]
- montar o conteúdo do arquivo .md da task com base em todas as respostas, seguindo exatamente a estrutura do template — o campo `grupo` vai no cabeçalho logo após `objetivo:`, nunca inline nas ações; sem comentários no arquivo final
